...
public class newnewpdf {
	...
	public void extraction(WebDriver driver, int index) throws InterruptedException, IOException {
		...
		if (index == 1) {
			...
			driver.findElement(By.className("registration-form__submit")).click();
			//driver.findElement(By.cssSelector("div.css-x9gf0u")).click();
		}
		
		//Thread.sleep(2000);
		
		String textname = driver.findElement(By.xpath(
				"/html[1]/body[1]/knowde-root[1]/knowde-main-layout[1]/section[1]/div[2]/knowde-storefront[1]/knowde-storefront-container[1]/div[1]/knowde-storefront-layout[1]/div[1]/div[1]/div[1]/div[1]/knowde-storefront-header-wrapper[1]/knowde-storefront-header[1]/div[1]/div[1]/div[2]/div[2]/h1[1]/span[1]"))
				.getText();
		...
		try {
			...
			System.out.println("Something went wrong in finding pdf button");

			//Thread.sleep(1000);
			try {
				JavascriptExecutor js2 = (JavascriptExecutor) driver;
				WebElement scroll2 = driver.findElement(By.xpath("(//knowde-product-summary[@class='ng-star-inserted'])[1]"));
				// js.executeScript("window.scrollBy(0,500)");
				...
				// String filepdf = "Technical_Information" + Integer.toString(index);
				// String filepdf2 = "Safety_Information" + Integer.toString(index);
	...
	public static void pdfdownload(String urlLink, File fileLoc, File fileLoc1) {
		try {
			...
			BufferedInputStream input1 = new BufferedInputStream(http.getInputStream());
			FileOutputStream ouputfile = new FileOutputStream(fileLoc);
			FileOutputStream ouputfile1 = new FileOutputStream(fileLoc1);
			BufferedOutputStream bufferOut = new BufferedOutputStream(ouputfile, 1024);
			BufferedOutputStream bufferOut1 = new BufferedOutputStream(ouputfile1, 1024);
			while ((readbyte = input.read(buffer, 0, 1024)) >= 0) {
				// Writing the content onto the file.
				bufferOut.write(buffer, 0, readbyte);
				//bufferOut1.write(buffer, 0, readbyte);
			...
			//bufferOut.close();
			//input.close();
			while ((readbyte = input1.read(buffer, 0, 1024)) >= 0) {
				// Writing the content onto the file.
				bufferOut1.write(buffer, 0, readbyte);
				//bufferOut1.write(buffer, 0, readbyte);
				// TotalDownload is the total bytes written onto the file.
